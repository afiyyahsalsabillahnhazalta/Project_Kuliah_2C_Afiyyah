...
if (!empty($_POST['input_pembelian_validate'])) {
    ...
    if (mysqli_num_rows($select) > 0) {
        ...
        $updateStok = mysqli_query($conn, "UPDATE tb_stokgudang SET qty = qty + '$qty' WHERE nama_barang = '$nama_barang'");

        if ($updateStok) {
            // Jika update stok berhasil, masukkan data pembelian
            $query = mysqli_query($conn, "INSERT INTO tb_pembelian (id, nama_barang, tgl_pembelian, qty, harga_satuan, jumlah_rp) VALUES ('$kode_pembelian', '$nama_barang', '$tgl_pembelian', '$qty', '$harga_satuan', '$jumlah_rp')");
